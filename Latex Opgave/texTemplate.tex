...
\textbf{Proof By Contradiction} 
\\ \\
Suppose that there exist some $i$ and $j$, $i < j$, in SA, $0 < i < j < |SA|$ and $len = S_0$ in $S=S_0$$, $S_0$ =\$, where $S[SA[i] + len] = \$$ and $S[SA[j] + len] = \$$. Suppose that SAIS-FLS contain one item, that would suggest that $i=j$ which is a contradiction.
\\ \\
...
\begin{figure}[H
    ...
    \label{fig:totalmalware}
\end{figure}
\subsection{Anti-malware Program}
The Anti-malware Program builds and maintain the data structures 

\subsection{Anti-malware Service}
...
\includepdf[pages={1-2}]{StartupScan.pdf}
\nocite{*}
